...
//修改自 week05-2 好玩的程式設計 兩層for(迴圈) 配合if .. else if .. else
void setup() {
...
void draw() {
  for (int i=0; i<10; i++) {//上週是 for y現在改成 for i 左手i
    for (int j=0; j<16; j++) {//上週是 for x現在改成 for j 左手j
      if ( a[i][j]==1) fill(#5DFF1F);//諾列陣有值, 設綠色
